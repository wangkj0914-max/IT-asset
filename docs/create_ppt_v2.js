...
const beforeImg = "data:image/png;base64," + fs.readFileSync("E:/it-asset-system/docs/before_excel.png").toString("base64");
const afterImg = "data:image/png;base64," + fs.readFileSync("E:/it-asset-system/docs/after_system.png").toString("base64");
...
// ===== SLIDE 2: BEFORE vs AFTER WITH IMAGES =====
const s2 = pres.addSlide();
s2.background = { color: WHITE };
s2.addShape(pres.shapes.RECTANGLE, { x: 0, y: 0, w: 10, h: 0.08, fill: { color: TEAL } });
s2.addText("Excel vs 系统 对比", { x: 0.6, y: 0.3, w: 8.8, h: 0.7, fontSize: 28, fontFace: "Arial Black", color: DARK, bold: true, margin: 0 });

// Before image (left)
s2.addImage({ data: beforeImg, x: 0.2, y: 1.15, w: 4.7, h: 2.65 });
s2.addShape(pres.shapes.RECTANGLE, { x: 0.2, y: 3.85, w: 4.7, h: 0.4, fill: { color: "DC2626" } });
s2.addText("X 分散的Excel文件，手工录入，无流程管控", { x: 0.4, y: 3.9, w: 4.4, h: 0.3, fontSize: 10, fontFace: "Calibri", color: WHITE, bold: true, margin: 0 });

// After image (right)
s2.addImage({ data: afterImg, x: 5.1, y: 1.15, w: 4.7, h: 2.65 });
s2.addShape(pres.shapes.RECTANGLE, { x: 5.1, y: 3.85, w: 4.7, h: 0.4, fill: { color: TEAL } });
s2.addText("V 统一的Web系统，数据集中，流程全闭环", { x: 5.3, y: 3.9, w: 4.4, h: 0.3, fontSize: 10, fontFace: "Calibri", color: WHITE, bold: true, margin: 0 });

// Bottom summary bar
const painItems = [
  "手工录入效率低易出错", "折旧人工算不准确", "状态不实时无追踪",
  "盘点靠纸质清单", "无领用归还闭环", "多站点隔离困难"
];
painItems.forEach(function(t, i) {
  s2.addShape(pres.shapes.RECTANGLE, { x: 0.2 + i * 1.62, y: 4.45, w: 1.52, h: 0.35, fill: { color: "FEF2F2" }, shadow: { type: "outer", blur: 2, offset: 1, angle: 135, color: "000000", opacity: 0.05 } });
  s2.addText(t, { x: 0.2 + i * 1.62, y: 4.45, w: 1.52, h: 0.35, fontSize: 9, fontFace: "Calibri", color: "991B1B", align: "center", valign: "middle", margin: 0 });
});

const gainItems = [
  "Web录入+批量导入", "自动折旧+EOL预警", "实时状态全追踪",
  "扫码盘点差异报告", "领用归还全闭环", "双站点完全隔离"
];
gainItems.forEach(function(t, i) {
  s2.addShape(pres.shapes.RECTANGLE, { x: 0.2 + i * 1.62, y: 4.95, w: 1.52, h: 0.35, fill: { color: LIGHT }, shadow: { type: "outer", blur: 2, offset: 1, angle: 135, color: "000000", opacity: 0.05 } });
  s2.addText(t, { x: 0.2 + i * 1.62, y: 4.95, w: 1.52, h: 0.35, fontSize: 9, fontFace: "Calibri", color: TEAL, align: "center", valign: "middle", margin: 0 });
});
